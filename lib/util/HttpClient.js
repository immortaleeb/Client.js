...
var Iterator = require('../iterators/Iterator'),
    ...
    zlib = require('zlib'),
    _ = require('lodash'),
    MeasureLogger = require('../measure/MeasureLogger');
...
HttpClient.prototype.request = function (url, method, headers, options) {
  ...
  responseIterator._read = function () {
    var item = this._source && this._source.read();
    if (item === null) {
      !this._doneLogging && MeasureLogger.logger && MeasureLogger.logger.logResponseSize(url, this._responseSize);
      this._doneLogging = true;
    } else {
      this._responseSize = (this._responseSize || 0) + item.length;
      this._push(item);
    }
  };
  ...
  function performRequest() {
    ...
    MeasureLogger.logger && MeasureLogger.logger.logRequest(url);
