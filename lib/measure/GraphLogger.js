...
var CSV_HEADERS = [ 'action', 'startTimestamp', 'endTimestamp', 'executionTime', 'data' ];
...
GraphLogger.prototype._createWriteStream = function () {
  this.writer = csvWriter({ headers: CSV_HEADERS });
};
...
GraphLogger.prototype._writeRow = function (action, data, timeResult) {
  this._writeArray([ action, timeResult.start, timeResult.end, timeResult.delta, data ]);
};
...
GraphLogger.prototype._writeDataToCsvFile = function () {
  var graphFile = path.join(this.logDir, 'graph' + this.graphIndex + '.csv');
  this.writer.pipe(fs.createWriteStream(graphFile));
};
