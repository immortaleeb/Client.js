  ...
  if (request) {
    if (request.isBound) {
      if (request.bindCount === 1) {
        logString = "SINGLE_BINDING(" + request.cost + ";" + request.boundNode.variable + ";" + request.boundNode.bindings.length + ")";
      } else {
        var vars = request.boundNodes.map(function (n) { return n.variable; });
        var bs = request.boundNodes.map(function (n) { return n.bindings.length; });
        logString = "DOUBLE_BINDING(" + request.cost + ";" + vars + ";" + bs + ")";
      }
    } else {
      logString = "DIRECT(" + request.cost + ")";
    }
    logString += " : " + tripleString(request.triple);
  }
  this._stop('calculating cheapest request', request && logString);
