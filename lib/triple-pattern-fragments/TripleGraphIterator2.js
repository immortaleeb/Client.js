...
function getNodeBindingsAsObjects(node) {
  return node.bindings.map(function (binding) {
    return Util.createObject(node.variable, binding);
  });
}
...
TripleGraphIterator.prototype._executeSingleBindingRequest = function (triplePattern, bindingsList, callback) {
  var self = this,
      allBindings = [],
      executedRequests = 0;
...
// Executes a request for each combination of the bindings in the two binding lists
TripleGraphIterator.prototype._executeDoubleBindingRequest = function (triplePattern, bindingsList1, bindingsList2, callback) {
  var bindingsList = [];
  bindingsList1.forEach(function (bindings1) {
    bindingsList2.forEach(function (bindings2) {
      var bindings = Util.extend(bindings1, bindings2);
      bindingsList.push(bindings);
    });
  });

  this._executeSingleBindingRequest(triplePattern, bindingsList, callback);
};
...
TripleGraphIterator.prototype._executeRequest = function (request, callback) {
  ...
  var graph = this._graph,
      ...
      bindingsCallback = function (bindings) {
        self.GraphLogger.stopRequestTriple(bindings);
  ...
  if (request.isBound) {
    if (request.bindCount === 2) {
      this._executeDoubleBindingRequest(
          request.triple,
          getNodeBindingsAsObjects(request.boundNodes[0]),
          getNodeBindingsAsObjects(request.boundNodes[1]),
          bindingsCallback
      );
    } else {
      this._executeSingleBindingRequest(
          request.triple,
          getNodeBindingsAsObjects(request.boundNode),
          bindingsCallback
      );
    }
  } else {
