...
var SparqlParser = require('sparqljs').Parser,
    ...
    GraphIterator = require('../query-optimization-graph/GraphIterator'),
...
function SparqlGroupIterator(source, group, options) {
  ...
  switch (group.type) {
  case 'bgp':
    if (options.algorithm === '2' || options.algorithm === 2)
      return new GraphIterator(source, group.triples, options);
    else
