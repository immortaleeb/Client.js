...
var Iterator = require('./Iterator'),
    TripleGraph = require('./TripleGraph'), // TODO
...
function TripleGraphIterator(pattern, options) {
  ...
  var graph = this._graph = new TripleGraph();
...
TripleGraphIterator.prototype._getCheapestRequest = function() {
  ...
  var tree = new TripleGraphOptimizationTree(this._graph),
...
TripleGraphIterator.prototype._read = function() {
  ...
  // TODO return all bindings for all variables in the graph one at a time
};

function extractRolesFromTriplePattern(triplePattern) {
  var roles = {};
  if (rdf.isVariable(triplePattern.subject)) { roles[triplePattern.subject] = 'subject'; }
  if (rdf.isVariable(triplePattern.object)) { roles[triplePattern.object] = 'object'; }
  if (rdf.isVariable(triplePattern.predicate)) { roles[triplePattern.predicate] = 'predicate'; }
  return roles;
}

// Converts bindings from a { '?var1': 'value' } format to { 'subject': 'value' } format
function graphBindingsToN3Bindings(bindings, variables) {
  if (typeof bindings.map === 'function')
    return bindings.map(function(bindings) { return graphBindingsToN3Bindings(bindings, variables); });

  var result = {};
  result[variables['subject']] = bindings.subject;
  result[variables['object']] = bindings.object;
  result[variables['predicate']] = bindings.predicate;
  return result;
}

// Converts bindings from a { 'subject': 'value' } format to a { '?var1': 'value' } format
function N3BindingsToGraphBindings(bindings, roles) {
  if (typeof bindings.map === 'function')
    return bindings.map(function(bindings) { return N3BindingsToGraphBindings(bindings); });

  var result = {};
  _.each(roles, function(variable, role) {
    var binding = bindings[variable];
    if (binding) result[role] = binding;
  });
  return result;
}
